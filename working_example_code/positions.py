...
def find_inconsistency(data,debug=0):
    ...
    if debug:
        print('theory ', theory)
    return (len([x for x in theory if x[0]=='BRR']),len([x for x in theory if x[0]=='FS']))
...
def find_inconsistency_alt(data,debug=0):
    ...
    if asa == 0:
        ...
        asadiv=asa
    cnx = len([x for x in theory if x[0]=='FS'])
...
def reconstruct_eam(data):
    ...
    print(f'Scores: Sticky {cb[0]:.3f} Context {cb[1]:.3f}')
    return cb
